...
procedure TFrmMain.FileCompareExecute(Sender: TObject);
Var
  ShCmdLn: STRING;
begin
...
//  ShCmdpm := 'TOP ' + sformTop + ' LEFT ' + sformLeft + ' HEIGHT ' + sformHeight  + ' WIDTH ' + sformWidth;
  ShellExecute(GetDesktopWindow(), NIL, PCHAR(ShCmdLn), '', NIL,
    SW_SHOWNORMAL);
end;
...
Begin
  MiniEdit.Enabled := FileExists(IncludeTrailingBackSlash(ExtractFilePath(Application.exename)) +
    'MINIEDIT.EXE');
    FileCompare.Enabled := FileExists(IncludeTrailingBackSlash(ExtractFilePath(Application.exename)) +
    'FILECOMPARE.EXE');
End;
